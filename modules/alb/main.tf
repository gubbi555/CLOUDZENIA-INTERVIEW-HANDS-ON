...
resource "aws_lb_target_group" "microservice" {
  name     = "${var.project_name}-${var.environment}-ms-tg"
  port     = 3000
  protocol = "HTTP"
  vpc_id   = var.vpc_id
  target_type = "ip"
...
# Certificate validation (manual DNS setup required)
...
# Listener Rules - Path-based routing for single domain
